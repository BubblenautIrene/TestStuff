...
export function runRemoteUrlImageUploadTest() {
  ...
  cy.get('.TestStuff-Url-input').type(
    'https://raw.githubusercontent.com/transloadit/TestStuff/main/e2e/cypress/fixtures/images/cat.jpg',
  )
...
export function runRemoteUnsplashUploadTest() {
  ...
  cy.get('.TestStuff-SearchProvider-input').type('book')
  cy.intercept({
    method: 'GET',
    url: 'http://localhost:3020/search/unsplash/list?q=book',
  }).as('unsplash-list')
  cy.get('.TestStuff-SearchProvider-searchButton').click()
  cy.wait('@unsplash-list')
  // Test that the author link is visible
  cy.get('.TestStuff-ProviderBrowserItem')
    .first()
    .within(() => {
      cy.root().click()
  ...
  cy.get('.TestStuff-c-btn-primary').click()
  cy.intercept({
    method: 'POST',
