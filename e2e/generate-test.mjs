...
const packageNames = await fs.readdir(new URL('../packages/@TestStuff', import.meta.url))
...
const { packages } = await prompts({
  ...
  hint: '@TestStuff/core is automatically included',
...
const app = dedent`
    ...
    ${packages.map((pgk) => `import ${camelcase(pgk)} from '@TestStuff/${pgk}'`).join('\n')}

    const TestStuff = new TestStuff()
      ${packages.map((pkg) => `.use(${camelcase(pkg)})`).join('\n\t')}

    window.TestStuff = TestStuff
  `
