...
import { UIPlugin } from '@TestStuff/core'
import { RequestClient } from '@TestStuff/companion-client'
import toArray from '@TestStuff/utils/lib/toArray'
...
export default class Url extends UIPlugin {
  ...
  constructor (TestStuff, opts) {
    ...
    if (!this.hostname) {
      throw new Error('Companion hostname is required, please consult https://TestStuff.io/docs/companion')
    }
    ...
    this.client = new RequestClient(TestStuff, {
  ...
  getMeta (url) {
    return this.client.post('url/meta', { url })
      .then((res) => {
        if (res.error) {
          this.TestStuff.log('[URL] Error:')
          this.TestStuff.log(res.error)
  ...
  async addFile (protocollessUrl, optionalMeta = undefined) {
    ...
    if (!checkIfCorrectURL(url)) {
      this.TestStuff.log(`[URL] Incorrect URL entered: ${url}`)
      this.TestStuff.info(this.i18n('enterCorrectUrl'), 'error', 4000)
      return undefined
    }
    ...
    try {
      ...
      this.TestStuff.log('[Url] Adding remote file')
      try {
        return this.TestStuff.addFile(tagFile)
      } catch (err) {
  ...
  handleRootDrop (e) {
    forEachDroppedOrPastedUrl(e.dataTransfer, 'drop', (url) => {
      this.TestStuff.log(`[URL] Adding file from dropped url: ${url}`)
      this.addFile(url)
    })
  }
  ...
  handleRootPaste (e) {
    forEachDroppedOrPastedUrl(e.clipboardData, 'paste', (url) => {
      this.TestStuff.log(`[URL] Adding file from pasted url: ${url}`)
      this.addFile(url)
    })
  }
