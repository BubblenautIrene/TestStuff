describe('Dashboard with Transloadit', () => {
  ...
  it('should upload cat image successfully', () => {
    ...
    cy.get('.TestStuff-StatusBar-actionBtn--upload').click()
    cy.wait(['@assemblies', '@resumable']).then(() => {
      cy.get('.TestStuff-StatusBar-statusPrimary').should('contain', 'Complete')
    })
  })
  ...
  it('should close assembly polling when cancelled', () => {
    ...
    cy.window().then(({ TestStuff }) => {
      ...
      cy.wait(['@createAssemblies']).then(() => {
        ...
        // @ts-ignore fix me
        expect(
          Object.values(TestStuff.getPlugin('Transloadit').activeAssemblies).every(
        ...
        cy.wait(['@delete']).then(() => {
          ...
          // @ts-ignore fix me
          expect(
            Object.values(TestStuff.getPlugin('Transloadit').activeAssemblies).some(
              (a: any) => a.pollInterval,
            ),
          ).to.equal(false)
        })
  ...
  it.skip('should emit one assembly-cancelled event when cancelled', () => {
    ...
    cy.window().then(({ TestStuff }) => {
      // eslint-disable-next-line
      // @ts-ignore fix me
      TestStuff.on('transloadit:assembly-cancelled', spy)
  ...
  it.skip('should close assembly polling when all files are removed', () => {
    ...
    cy.window().then(({ TestStuff }) => {
      ...
      const { files } = TestStuff.getState()
      // eslint-disable-next-line
      // @ts-ignore fix me
      TestStuff.removeFiles(Object.keys(files))
  ...
  it('should not create assembly when all individual files have been cancelled', () => {
    ...
    cy.window().then(({ TestStuff }) => {
      ...
      // @ts-ignore fix me
      expect(
        Object.values(TestStuff.getPlugin('Transloadit').activeAssemblies).length,
      ).to.equal(0)
  ...
  it('should not emit error if upload is cancelled right away', () => {
    ...
    cy.window().then(({ TestStuff }) => {
      const { files } = TestStuff.getState()
      TestStuff.on('upload-error', handler)
  ...
  it('should not re-use erroneous tus keys', () => {
    function createAssemblyStatus({
      ...
      return {
        ...
        TestStuffserver_url: 'https://api2-test.transloadit.com/companion/',
        ...
        client_referer: null,
        transloadit_client:
          'TestStuff-core:3.2.0,TestStuff-transloadit:3.1.3,TestStuff-tus:3.1.0,TestStuff-dropbox:3.1.1,TestStuff-box:2.1.1,TestStuff-facebook:3.1.1,TestStuff-google-drive:3.1.1,TestStuff-instagram:3.1.1,TestStuff-onedrive:3.1.1,TestStuff-zoom:2.1.1,TestStuff-url:3.3.1',
    ...
    cy.get('.TestStuff-StatusBar-statusPrimary').should('contain', 'Upload failed')
    ...
    // SECOND ATTEMPT
    cy.get('.TestStuff-StatusBar-actions > .TestStuff-c-btn').click()
    cy.wait([
